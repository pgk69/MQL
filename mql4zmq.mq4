...
////////// Receive subscription data via main API //////////
  ...
  int request[1];                                          // Initialize message.
  ...
  if (zmq_recv(listener, request, ZMQ_NOBLOCK) != -1) {    // Will return -1 if no message was received.
    ...
    if (message_length) {
      Print("Received message: " + message);               // Print message, if there is one.
    }
  }
...
////////// to do the same thing using the helpers instead.
  ...
  string current_tick = "tick " + Bid + " " + Ask + " " + Time[0]; // Publish current tick value.
...
////////// Publish data via main API //////////
  ...
  zmq_msg_close(response);                                 // Deallocate message.
